...
function chooseProduct(id) {
  ...
  if (!isCheckoutPage()) {
    window.location.href = `./checkout.html?product=${encodeURIComponent(id)}`;
    return;
  }
  renderProductOptions();
  updateSelectedProduct();
...
function updateCheckoutTotal() {
  ...
  if (product && refreshedSubtotalValue) refreshedSubtotalValue.textContent = money(product.price);
  if (product && refreshedShippingValue) refreshedShippingValue.textContent = getEffectiveDeliveryText();
  if (product && refreshedTotalValue) refreshedTotalValue.textContent = money(product.price + getEffectiveDeliveryFee()).replace(" EGP", "");
